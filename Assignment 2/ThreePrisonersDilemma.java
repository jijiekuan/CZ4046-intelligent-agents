public class ThreePrisonersDilemma {
	...
	class CalculativePlayer extends Player {
		// We track the number of tats received
		int tats = 0;
		
		int selectAction(int n, int[] myHistory, int[] oppHistory1, int[] oppHistory2) {
			
			// We cooperate by default
			if (n==0) return 0; 
			
			 
			switch(oppHistory1[n-1]+oppHistory2[n-1]) {
				// Both opponents coop, we coop
				case 0:
					// Drop grudges when opponents coop
					tats = 0;
					return 0;
				// Either opponent defect, we perform tit for 2 tats
				case 1:
					if (tats>=2) {
						return 1;
					}
				...
				// Both opponents defect, we defect
				default:
					return 1;
...
//	This is a helper function needed by scoresOfMatch.
	...
	Player makePlayer(int which) {
		...
		case 4: return new CalculativePlayer();
		case 5: return new RandomPlayer();
